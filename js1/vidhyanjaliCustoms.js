...
(function ($) {
  ...
  let swtichSlider = function() {
    $('#eventsContainer').on('click', '.events', function (event) {
      ...
      $('#eventsCarousels .arrow1').removeClass('active');
      $('#eventsCarousels .arrow1:eq('+ parentElementNo +')').addClass('active').find('.vi_slider').nivoSlider();
    });
  };
  swtichSlider();
  // On page load start the default slider for active arrow box.
  $('#eventsCarousels .arrow1.active .vi_slider').nivoSlider();
